...
function migrateBrokenImages(store: StoreData): boolean {
  // Never remove or overwrite saved image URLs in data/store.json
  return false;
}
    ...
    if (fs.existsSync(STORE_FILE)) {
      ...
      const products = rawProducts.map((p: any) => {
        ...
        return {
          ...
          material: typeof p.material === 'string' ? p.material : (p.material || ''),
...
app.post('/api/products', requireAdmin, (req, res) => {
  ...
  const newProduct = {
    ...
    material: typeof productData.material === 'string' ? productData.material : (productData.material || ''),
    colors: Array.isArray(productData.colors) ? productData.colors : (Array.isArray(productData.colorOptions) ? productData.colorOptions : []),
    colorOptions: Array.isArray(productData.colorOptions) ? productData.colorOptions : (Array.isArray(productData.colors) ? productData.colors : []),
